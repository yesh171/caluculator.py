from tkinter import *
root=Tk()
#tittle of the caluculator
root.title("yeshwanth caluculator")
...
def equal():
    ...
    if math=="add":
        ...
        e.insert(0,fnum**int(secondnum))
#function that calculate global is used to access another function
def sub():
    firstnum=e.get()
    global fnum
    global math
...
buttoneqal=Button(root, text="=", padx=40, pady=20, command=equal)
...
buttoneqal.grid(row=6, column=0)
buttonpow.grid(row=6, column=1)
buttonquit.grid(row=6, column=2)
#without this ths application couldnt run
root.mainloop()
